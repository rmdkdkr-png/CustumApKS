#!/usr/bin/env python3
"""tilemap.py — NGPC 화면 타일 관측기 + 오프라인 시뮬레이터.

용도
  1) 관측: 게임을 돌리며 각 화면의 VRAM을 떠서
     "이 화면에 실제로 뜬 ROM 타일 주소" 를 전부 기록한다.
     → 정적 아이템 스캐너가 놓치는 타일(창 밖 이주, 아이템 미경유 직접판독)까지 잡힌다.
...
def dump_vram(ngp):
    return ngp.read(0x8000, 0x4000)


def tile_table(vram):
    T = np.zeros((NTILE, 8, 8), np.uint8)
    for s in range(NTILE):
        o = TILE_BASE - 0x8000 + s * 16
        for r in range(8):
...
def observe(vram, rom_idx):
    ...
    for p, base in SCR.items():
        ...
        for r in range(32):
            for c in range(32):
                v = struct.unpack_from('<H', vram, base - 0x8000 + (r * 32 + c) * 2)[0]
                m[r, c] = v
                used.add(v & 0x1FF)
        maps[p] = m
    slot2rom = {}
...
def render(maps, tiles, plane=None):
    """타일맵 + 타일표 -> 회색조 이미지(256x256). plane=None이면 두 평면 가로로 붙임."""
    def one(m):
        img = np.zeros((256, 256, 3), np.uint8)
        for r in range(32):
            for c in range(32):
                img[r * 8:r * 8 + 8, c * 8:c * 8 + 8] = GRAY[tiles[m[r, c] & 0x1FF]]
        return img
    if plane:
